// pages/components/chooseCity.js
var utils = require("../../utils/util.js");

function array_contain(array, obj) {
  for (var i = 0; i < array.length; i++) {
    if (array[i].adcode == obj.adcode) //如果要求数据类型也一致，这里可使用恒等号===
      return true;
  }
  return false;
}
...
Component({
  ...
  methods: {
    ...
    resetRight(data) {
      ...
      var recently = wx.getStorageSync("recently") || [];
      var recentData = {
        title: "最近访问",
        type: 'hot',
        item: recently
      }
      this.data.data.unshift(recentData)
    ...
    detailMt(e) {
      ...
      var recentlyData = wx.getStorageSync("recently") || [];

      if (!array_contain(recentlyData, detail)) {
        recentlyData.push(detail)
      }

      if (recentlyData.length > 8) {
        recentlyData.splice(0, recentlyData.length - 8);
      }
      wx.setStorage({
        key: 'recently',
        data: recentlyData,
      })
      console.log("Storage", recentlyData);
    ...
    _search() {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        for (let j = 0; j < data[i].item.length; j++) {
          if (data[i].item[j].city_name.indexOf(this.value) > -1) {
            let itemJson = {};
    ...
    locationMt() {
      ...
      var that = this;
      if (this.data.data[0].title != '我的定位') {
        ...
        success: function(res) {
          ...
          utils.getAddress(res.latitude, res.longitude, {
            success(e) {
              ...
              var city = e.originalData.result.addressComponent.city;
              var location = e.originalData.result.location
              // var lat =
              console.log(e);

              that.data.data[0].item[0]["city_name"] = city;
              that.data.data[0].item[0]["adcode"] = code;
              that.data.data[0].item[0]["lng"] = location.lng;
              that.data.data[0].item[0]["lat"] = location.lat;
